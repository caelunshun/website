...
pub fn with_state<S: Clone + Send + 'static>(state: S) -> impl Filter<Extract=(S,), Error=Infallible> + Clone {
...
pub fn authenticated(db: DB) -> impl Filter<Extract = (u32,), Error = Rejection> + Clone {
    warp::header::header("Authorization").and_then(move |token: String| {
        ...
        async move {
            ...
            let user_id = db.get_user_id_by_auth_token_secret(secret.as_ref())
            .await
