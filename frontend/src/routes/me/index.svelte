		...
		<div class="container mx-auto flex flex-col px-4 my-8">
			...
			<div class="flex flex-col">
				...
				<div>
					...
					<p class="leading-6">
						If you want to use plugin commands from the command line, you will need to login with <code
							>cargo quill login (token)</code
						> using one of the tokens listed below.
					</p>
					<p class="leading-6">
						When working in shared environments, supplying the token on the command line could
						expose it to prying eyes. To avoid this, enter <code>cargo quill login</code> and supply
						your token when prompted.
					</p>
						...
						{#each tokens as { name, used_total, created_at, token }}
							<li class="mt-4">
								<Card>
									<div class="flex justify-between">
										...
										<div class="flex">
											<div class="flex flex-col text-sm font-light mr-4">
												<span>Created {timeago.simple(new Date(created_at))}</span>
												{#if used_total > 0}
									...
									{#if token}
										<p>
											Please record this token somewhere, you cannot retrieve its value again. For
											use on the command line you can save it to ~/.cargo/quill with:
										</p>
