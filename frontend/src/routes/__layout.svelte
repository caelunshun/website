<script lang="ts">
    import "../global.css";
	import { token, preferences } from "$lib/stores/local";
...
>
	<nav
		class="md:flex flex-1 justify-between items-center md:h-24 py-4 px-4 container mx-auto relative {isDropdownShown
			? 'max-h-96'
	...
	>
		...
		>
		<ul
			class="md:flex items-center md:space-x-6 space-y-4 md:space-y-0 mt-4 md:mt-0 text-normal sm:text-xl font-bold {isDropdownShown
				? ''
				: 'hidden md:block'}"
		>
			...
			{:else}
				...
				<li>
					<a href="/me/login" class="flex items-center">
						<LockIcon class="h-6 w-6" />
						<span class="mx-2 my-auto">Login (GitHub)</span></a
					>
				</li>
			{/if}
		</ul>
		<div
			class="absolute transition-transform right-6 top-9 md:hidden z-50 text-white transform -translate-x-1/2 -translate-y-1/2"
			on:click={() => {
...
<main
	class="flex flex-col flex-1 transition-colors duration-200 {$preferences.dark
...
>
	...
	<div
		class="w-16 h-8 flex items-center mx-auto sm:mx-2 my-2 sm:my-0 {$preferences.dark
			? 'bg-gray-900'
	...
	>
		<div
			class="w-7 h-7 rounded-full shadow-md transform bg-white my-auto mx-1 {$preferences.dark
				? 'translate-x-7'
