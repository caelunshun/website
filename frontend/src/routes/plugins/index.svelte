...
<div class="container mx-auto flex flex-1 p-4 flex-col-reverse lg:flex-row">
	<div class="flex flex-1 flex-col my-4">
		<form
			class="flex justify-between items-center border border-feather-light dark:border-nether-lava focus-within:border-green-400 dark:focus-within:border-nether-glowstone w-full rounded text-xl px-4 py-2"
		>
			<input
				class="flex-1 bg-transparent outline-none"
				role="search"
				aria-label="Seach for plugins"
				type="text"
				placeholder="Search..."
				bind:value={search}
			/>
			<SearchIcon class="ml-4 h-6 w-6 text-feather-dark dark:text-nether-lava cursor-pointer" />
		</form>
	...
	<aside class="border-0 lg:border-l-2 m-0 lg:mx-8 lg:pl-4 my-4">
		...
		<button
			...
			class="text-sm font-light lg:hidden hover:text-feather-dark"
		>
			{#if show_categories}
				Hide categories
			{:else}
		...
		<ul
			class="flex flex-wrap lg:flex-nowrap lg:flex-col {!show_categories
				? 'hidden'
				: 'block'} lg:block"
		>
