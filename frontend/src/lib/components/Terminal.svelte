<script lang="ts">
    ...
    async function createTerm() {
        ...
        for (let output of outputs) {
            ...
            curcallbackstack +=
                timeout + (output.typewriter ? output.message().length * 105 + 1000 : 0);
        }
    }
...
<div
    id="term"
    class="w-11/12 lg:w-7/12 mx-auto font-mono"
    on:mouseenter={() => {
        ismultiplierselshown = true;
    }}
    on:mouseleave={() => {
        ismultiplierselshown = false;
    }}
>
    ...
    <div
        id="termscreen"
        class="w-full mx-auto p-0.5 rounded-b-lg overflow-y-auto relative"
        style="height: 30rem;"
    >
        ...
        <div
            id="termsel"
            class="{ismultiplierselshown
                ? 'opacity-100'
                : 'opacity-0'} border border-gray-200 transform -translate-y-1/2 -translate-x-1/2 w-10/12 transition-opacity duration-1000 rounded-xl absolute -bottom-4 left-1/2"
        >
            <strong class="text-center mt-2 block" style="color: #fff;"
                >Slowmotion (decrease the speed of the "process" output)</strong
            >
